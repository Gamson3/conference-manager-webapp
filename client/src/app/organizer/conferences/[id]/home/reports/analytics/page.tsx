...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const [submissionsRes, participantStatsRes, programStatsRes, sessionsRes] = await Promise.allSettled([
        apiClient.get(API_ENDPOINTS.ORGANIZER.SUBMISSIONS(conferenceId)),
        apiClient.get(API_ENDPOINTS.ORGANIZER.PARTICIPANT_STATS(conferenceId)),
      ...
      submissions.forEach((sub: { status: string; category?: { name: string }; type?: { name: string }; submittedAt: string }) => {
        byStatus[sub.status] = (byStatus[sub.status] || 0) + 1;
        if (sub.category?.name) {
          categoryMap.set(sub.category.name, (categoryMap.get(sub.category.name) || 0) + 1);
        }
        if (sub.type?.name) {
          typeMap.set(sub.type.name, (typeMap.get(sub.type.name) || 0) + 1);
        }
      ...
      const participantData = participantStatsRes.status === "fulfilled" ? participantStatsRes.value.data : null;
      const byRole: Record<string, number> = participantData?.byRole || {};
      const participantsByStatus: Record<string, number> = participantData?.byStatus || {
        registered: participantData?.registered || 0,
        waitlisted: participantData?.waitlisted || 0,
        canceled: participantData?.canceled || 0,
      };
      ...
      const acceptedSubmissions = typeof programStats.acceptedSubmissions === "number" ? programStats.acceptedSubmissions : accepted;
      const scheduledPresentations = typeof programStats.presentationsCount === "number" ? programStats.presentationsCount : 0;
      const fillRate = acceptedSubmissions > 0 ? Math.round((scheduledPresentations / acceptedSubmissions) * 100) : 0;

      setAnalytics({
        overview: {
          totalSubmissions,
          totalParticipants: participantData?.total || 0,
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(analytics.participants.byRole).map(([role, count]) => {
                const roleColors: Record<string, { bg: string; text: string }> = {
                  attendee: { bg: "bg-blue-500/10", text: "text-blue-500" },
                  presenter: { bg: "bg-purple-500/10", text: "text-purple-500" },
                  author: { bg: "bg-green-500/10", text: "text-green-500" },
                  reviewer: { bg: "bg-amber-500/10", text: "text-amber-500" },
                  sponsor: { bg: "bg-pink-500/10", text: "text-pink-500" },
                  volunteer: { bg: "bg-cyan-500/10", text: "text-cyan-500" },
                  organizer: { bg: "bg-orange-500/10", text: "text-orange-500" },
                };
