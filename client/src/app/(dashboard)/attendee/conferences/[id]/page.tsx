...
export default function ConferenceDetails() {
  ...
  useEffect(() => {
    const fetchConferenceDetails = async () => {
      try {
        ...
        const registrationResponse = await api.get(`/attendee/check-registration/${conferenceId}`);
        setIsRegistered(registrationResponse.data.isRegistered);
        
      } catch (error: any) {
        console.error('Error fetching conference details:', error);
        setError(error.response?.data?.message || 'Failed to load conference details');
  ...
  const handleRegister = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to register for conference");
    }
  };
  ...
  const isEventActive = new Date() >= new Date(conference.startDate) && new Date() <= new Date(conference.endDate);
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{conference.language || 'English'}</p>
                    </div>
                ...
                {conference.schedule.map((day, dayIndex) => (
                  <div key={dayIndex} className="mb-8">
                    <h3 className="mb-4 bg-primary-50 p-3 rounded-md font-medium">
                      Day {dayIndex + 1}: {new Date(day.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                    </h3>
                                  ...
                                  {conference.speakers && (
                                    <div className="flex items-center">
                                      ...
                                      {session.speakers.map(speakerId => {
                                        const speaker = conference.speakers?.find(s => s.id === speakerId);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {conference.speakers && conference.speakers.length > 0 ? (
                ...
                <div className="col-span-full text-center py-12 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-gray-500">
                    Speaker information for this conference has not been added yet.
                  </p>
                </div>
              )}
            </div>
