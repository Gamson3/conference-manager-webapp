...
export default function PublishEventPage() {
  ...
  const fetchEventData = async () => {
    ...
    try {
      ...
      const [eventRes, categoriesRes, typesRes, submissionRes] = await Promise.all([
        api.get(`/events/${eventId}`),
        api.get(`/api/events/${eventId}/categories`).catch(() => ({ data: [] })),
        api.get(`/api/events/${eventId}/presentation-types`).catch(() => ({ data: [] })),
        api.get(`/api/events/${eventId}/submission-settings`).catch(() => ({ data: null }))
      ]);
      ...
      if (submissionData) {
        setSubmissionSettings({
          submissionDeadline: submissionData.submissionDeadline?.split('T')[0] || '',
          allowLateSubmissions: submissionData.allowLateSubmissions,
          requireAbstract: submissionData.requireAbstract,
          maxAbstractLength: submissionData.maxAbstractLength,
          requireFullPaper: submissionData.requireFullPaper,
          allowedFileTypes: submissionData.allowedFileTypes,
          maxFileSize: submissionData.maxFileSize,
          reviewProcess: submissionData.reviewProcess,
          requireAuthorBio: submissionData.requireAuthorBio,
          requireAffiliation: submissionData.requireAffiliation,
          notificationEmails: submissionData.notificationEmails,
          submissionGuidelines: submissionData.submissionGuidelines,
          enableSubmissions: submissionData.enableSubmissions
        });
  ...
  const validateConfiguration = (
    ...
    if (categoriesData.length === 0) errors.push('At least one category is required');
    if (typesData.length === 0) errors.push('At least one presentation type is required');
  ...
  const handlePublishWithSubmissions = async () => {
    ...
    try {
      ...
      await api.put(`/api/events/${eventId}/submission-settings`, submissionSettings);
      ...
      toast.error(error.response?.data?.message || 'Failed to publish conference');
    } finally {
      setPublishing(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          <Button 
            variant="ghost" 
            onClick={() => router.push(`/organizer/create-event/categories?eventId=${eventId}`)}
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Publish Conference & Configure Submissions</h1>
            <p className="text-gray-600 mt-2">
              Set up submission guidelines and publish your conference to start accepting papers.
            </p>
          </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <br />
                  <span>{event?.startDate ? new Date(event.startDate).toLocaleDateString() : 'Not set'}</span>
                </div>
                <div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="text-center p-4 bg-purple-50 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-purple-600">{presentationTypes.length}</div>
                  <div className="text-sm text-purple-700">Presentation Types</div>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setSubmissionSettings(prev => ({
                    ...prev,
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="require-abstract">Require Abstract</Label>
                  <Switch
                    id="require-abstract"
                    checked={submissionSettings.requireAbstract}
                    onCheckedChange={(checked) => setSubmissionSettings(prev => ({
                      ...prev,
                      requireAbstract: checked
                    }))}
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="require-paper">Require Full Paper</Label>
                  <Switch
                    id="require-paper"
                    checked={submissionSettings.requireFullPaper}
                    onCheckedChange={(checked) => setSubmissionSettings(prev => ({
                      ...prev,
                      requireFullPaper: checked
                    }))}
                  />
                </div>
      ...
      {isValid && (
        <motion.div
          ...
          className="mt-8 p-6 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-green-800">Ready to Accept Submissions!</h3>
              <p className="text-sm text-green-700 mt-1">
                Your conference will be published with a "Call for Papers" status. Authors can start submitting their presentations immediately.
              </p>
            </div>
          </div>
        </motion.div>
