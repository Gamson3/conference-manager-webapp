...
async function main() {
  ...
  // Base users (previously attendees)
...
Join us for three days of inspiring presentations, workshops, and networking opportunities with the brightest minds in AI.`,
  ...
  // 7. Create participants (registrations) for attendees
  ...
  // Add organizers as participants as well
  await prisma.conferenceParticipant.create({
    data: { userId: organizer1.id, conferenceId: conference.id, role: 'attendee', status: 'registered' }
  });
  await prisma.conferenceParticipant.create({
    data: { userId: organizer2.id, conferenceId: conference.id, role: 'attendee', status: 'registered' }
  });
...
- Participants: ${await prisma.conferenceParticipant.count()}
