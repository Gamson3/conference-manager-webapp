...
export const getSectionsByConference = async (req: Request, res: Response): Promise<void> => {
  try {
    const conferenceIdRaw = req.params.conferenceId ?? req.params.id;
    if (!conferenceIdRaw) {
      sendError(res, 400, "INVALID_ARGUMENT", "Conference ID is required");
      return;
    }

    const parsedConferenceId = parseNumericId(conferenceIdRaw, "Conference ID");
    if (!parsedConferenceId.ok) {
      sendError(res, 400, "INVALID_ARGUMENT", parsedConferenceId.message);
      return;
    }

    const conferenceId = parsedConferenceId.value;
    ...
    if (!isAdmin(req) && conference.createdById !== getUserId(req)) {
       sendError(res, 403, "FORBIDDEN", "You do not have permission to view sessions for this conference.");
       return;
    }
    ...
    // Map sections to ensure dayId is explicitly included
    const formattedSections = sections.map(section => {
      const { day, ...sectionData } = section;
      return {
        ...sectionData,
        dayId: section.dayId,
        day: day || null
    ...
    console.error('Error in getSectionsByConference:', error);
    sendError(res, 500, "INTERNAL", "Failed to load sessions.");
  }
};
