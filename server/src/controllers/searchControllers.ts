...
//         id: { not: undefined } // ✅ Ensure section exists
//       }
//     };
...
// // For attendees - only scheduled presentations
// export const searchScheduledPresentations = async (req: Request, res: Response) => {
//   try {
//     const { id } = req.params;
//     const { q, type, limit = 50 } = req.query;
//     const userId = getUserId(req);
...
//       section: {
//         conferenceId: Number(id),
//         id: { not: null } // Only scheduled presentations
//       }
//     };
...
//         whereClause.section = {
//           conferenceId: Number(id),
//           id: { not: null }, // Only scheduled sections
//           name: textFilter
//         };
//         break;
...
//   } catch (error: any) {
//     console.error('Error searching scheduled presentations:', error);
//     res.status(500).json({ message: 'Search failed', error: error.message });
//   }
// };

// // For organizers - all presentations (scheduled + unscheduled)
// export const searchAllPresentations = async (req: Request, res: Response) => {
//   try {
//     const { id } = req.params;
//     const { q, type, limit = 50, includeUnscheduled = 'false' } = req.query;
//     const userId = getUserId(req);
...
//     if (!conference && !isAdmin(req)) {
//       res.status(403).json({ message: "Only organizers can search all presentations" });
//       return;
//     }

//     let whereClause: Prisma.PresentationWhereInput = {
//       conferenceId: Number(id) // Search by conferenceId directly, not through section
//     };
...
//         section: { include: { day: true } }, // Optional section
//         category: true,
...
//     // Format results handling null sections
//     const formattedResults = presentations.map(presentation => ({
//       id: presentation.id,
//       title: presentation.title,
//       abstract: presentation.abstract,
...
//         isPresenter: author.isPresenter
//       })),
//       category: presentation.category?.name,
//       presentationType: presentation.presentationType?.name,
//       // Handle optional section
//       scheduling: presentation.section ? {
//         isScheduled: true,
//         section: {
//           id: presentation.section.id,
//           name: presentation.section.name,
...
//   } catch (error: any) {
//     console.error('Error searching all presentations:', error);
//     res.status(500).json({ message: 'Search failed', error: error.message });
//   }
// };